...
if pgrep -x "wlsunset" > /dev/null; then
    # 如果正在运行，则关闭
    notify-send -t 2000 -a "System" "Eye Protection Wlsunset" "Closed"
    pkill -x wlsunset
else
    # 如果未运行，则开启
    ...
    wlsunset -S 07:00 -s 19:00 -t $TEMP_LOW -T $TEMP_HIGH -d $TRANSITION &
    notify-send -t 2000 -a "System" "Eye Protection Wlsunset" "Opened with ${TEMP_LOW}K"
fi
